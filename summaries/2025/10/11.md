...
## 4:37:49 PM
The provided log details significant updates to the `RPI_Server` component of the `MDP-Maxxing` project, specifically focusing on its `Week9.py` script and a `plan.txt` file.

**`c:\Users\zhiha\MDP_Shared\MDP-Maxxing\RPI_Server\Week9.py` (Timestamp: 10/11/2025, 3:38:59 PM)**
This Python script establishes the core functionality of a Raspberry Pi server, orchestrating communication with an Android device, an STM microcontroller, and handling image processing. Key aspects include:
*   **System Architecture**: Implements a multi-threaded and multi-process design using `threading` and `multiprocessing.Manager` for robust concurrent operations and inter-process communication. It initializes `AndroidLink` and `STMLink` for device interactions.
*   **Camera Integration**: Features conditional `PiCamera` setup, enabling image capture (`take_snapshot`) for Raspberry Pi environments, and setting a fixed resolution of 640x640.
*   **Communication Flow**:
    *   **Android**: Manages a dedicated process (`android_io_process`) for connecting, receiving (`recv_android`), and sending (`send_android`) messages. It includes logic for reconnection attempts upon connection loss. Specific Android commands handled include `sendArena` (triggering a `<START>` command to STM) and `stitch-image` (queuing a `PiAction` for image stitching).
    *   **STM Microcontroller**: Communicates with an STM device, sending commands and waiting for acknowledgments (`send_and_wait`). The `stm_queue_listener` processes commands destined for STM. The `send_and_wait` logic has been refined to primarily wait for an `stm_received` event, indicating the STM has acknowledged command reception.
*   **Image Recognition and Action**: A crucial feature is the handling of the `<SNAPS>` command. Upon receiving this from STM, the RPI captures an image using the `take_snapshot` method, uploads it to a configured `IMAGE_SERVER` for processing. Based on the server's response, if specific object labels (e.g., left arrow (39) or right arrow (38)) are detected, corresponding commands (`<LEFTS>`, `<RIGHT>`) are sent back to the STM, facilitating a feedback loop for navigation or interaction. The system also tracks `obstacle_num` and sends status updates to Android regarding image capture.
*   **Queues and Events**: Extensive use of `multiprocessing.Manager` queues (`android_queue`, `rpi_action_queue`, `stm_queue`) and `threading.Event`s (`stm_done`, `stm_received`, `stm_stop_event`, `android_stop_event`) is central to managing asynchronous tasks and coordinating between different system components and processes.
*   **Error Handling**: Includes `try-except` blocks for various operations, particularly for communication links, ensuring resilience against connection drops or hardware failures, with mechanisms for logging and reattempting connections.
*   **Environmental Variables**: Utilizes `load_dotenv()` to load configuration, specifically `IMAGE_SERVER_URL`.

**`c:\Users\zhiha\MDP_Shared\MDP-Maxxing\RPI_Server\plan.txt` (Timestamp: 10/11/2025, 3:48:10 PM)**
This plain text file provides specifications for sensors relevant to the project. It details the maximum detection range for a front ultrasound sensor (90cm) and the detection range for side IR sensors (10cm - 40cm). This information likely serves as a reference for hardware configuration, path planning, obstacle detection algorithms, or overall system design.

**Patterns/Recurring Elements**:
A clear pattern emerging from these changes is the emphasis on robust inter-device communication (Android-RPI-STM) and the integration of advanced perception capabilities (camera-based image recognition via an external server) to drive robot actions. The consistent use of queues and events forms a core architectural choice for managing asynchronous data flow and control signals across different processes and threads. There's also a strong focus on error recovery and system stability, particularly concerning communication links. The project appears to be developing an intelligent system capable of sensing its environment, processing visual data, and autonomously making decisions based on perceived objects.